...
import { TeamPlayer } from './schemas/teamPlayer.schema';
...
export class TeamPlayerController {
  ...
  @Post()
  async create(@ModifyBody(setCreatedBy()) createTeamPlayerDto: TeamPlayer) {
  ...
  async patch(
    @Query() query,
    @Body() patchTeamPlayerDto: Partial<TeamPlayer>,
    @Param('id') id,
  ) {
